# Vector2.UnitX Property

﻿Vector2.UnitX Property 

|  |  |  |
| --- | --- | --- |
| Vector2UnitX Property | RAGE Plugin Hook Documentation |  |

[This is preliminary documentation and is subject to change.]

Gets the X unit [Vector2](T_Rage_Vector2.htm) (1, 0).

**Namespace:**
 [Rage](N_Rage.htm)  
**Assembly:**
 RagePluginHook (in RagePluginHook.dll) Version: 0.0.0.0 (0.56.1131.11510)

Syntax

[C#](#)

[VB](#)

[C++](#)

[Copy](# "Copy")

```
public static Vector2 UnitX { get; }
```

```
Public Shared ReadOnly Property UnitX As Vector2
	Get
```

```
public:
static property Vector2 UnitX {
	Vector2 get ();
}
```

#### Property Value

Type: [Vector2](T_Rage_Vector2.htm)  
A [Vector2](T_Rage_Vector2.htm) that has a value of (1, 0).

See Also

#### Reference

[Vector2 Structure](T_Rage_Vector2.htm)

[Rage Namespace](N_Rage.htm)

This version of the documentation applies to RAGE Plugin Hook v0.56.1131.11508 ALPHA

Copyright 2015-2017 © MulleDK19 / LMS ® All rights reserved.