# Camera.Delete Method

﻿Camera.Delete Method 

|  |  |  |
| --- | --- | --- |
| CameraDelete Method | RAGE Plugin Hook Documentation |  |

[This is preliminary documentation and is subject to change.]

Deletes this instance.

Any call to a non-existent instance is invalid and will cause an exception.

**Namespace:**
 [Rage](N_Rage.htm)  
**Assembly:**
 RagePluginHook (in RagePluginHook.dll) Version: 0.0.0.0 (0.56.1131.11510)

Syntax

[C#](#)

[VB](#)

[C++](#)

[Copy](# "Copy")

```
public override sealed void Delete()
```

```
Public Overrides NotOverridable Sub Delete
```

```
public:
virtual void Delete() override sealed
```

#### Implements

[IDeletableDelete](M_Rage_IDeletable_Delete.htm)  

See Also

#### Reference

[Camera Class](T_Rage_Camera.htm)

[Rage Namespace](N_Rage.htm)

This version of the documentation applies to RAGE Plugin Hook v0.56.1131.11508 ALPHA

Copyright 2015-2017 © MulleDK19 / LMS ® All rights reserved.